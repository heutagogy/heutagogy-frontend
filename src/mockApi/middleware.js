...
const isMockRSAA = action =>
  isPlainObject(action) && action.hasOwnProperty(CALL_MOCK_API)
      ...
      return new Promise(resolve => {
        setTimeout(() => {
          const nextAction = next({
            ...successType,
            payload:
              typeof successType.payload === 'function'
                ? successType.payload(action, getState(), {
                    ...res,
                    json: () => ({
                      then: callback => callback(res.responseJSON)
                    }),
                    headers: {
                      get: () => 'json'
                    }
