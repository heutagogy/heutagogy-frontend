...
const isSuccessAction = action =>
  !action.error && action.type.endsWith('_SUCCESS')

const isStartAction = action =>
  !action.error &&
  (action.type.endsWith('_START') || action.type.endsWith('_STARTED'))
...
const invalidateViewIds = (state, action) => {
  ...
  return state.mergeDeep(
    viewIds.reduce(
      (previous, current) =>
        previous.set(current, new Immutable.Map({ invalidated: true })),
      new Immutable.Map()
    )
  )
}
