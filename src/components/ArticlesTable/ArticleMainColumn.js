...
export class ArticleMainColumn extends Component {
  ...
  handleTagsChange(e, newTags) {
    this.setState({
      newTags:
        newTags !== '' ? newTags.split(',').map(s => s.replace(/ /g, '')) : []
    })
  }
  ...
  render() {
    return (
      ...
      >
        <div
          className={
            this.props.read ? styles.linkDivRead : styles.linkDivUnread
          }
        >
            ...
            <div>
              ...
              <div style={{ marginBottom: '5px' }}>
                {this.props.tags.map((tag, i) => (
                  <span key={`${tag}${i}`} style={inlineStyles.tag}>
                    {tag}
                  </span>
                ))}
              </div>
